...
func TestRegisterAndList(t *testing.T) {
	...
	views := r.List("user")
	if len(views) != 1 || views[0].Username != "alice" || views[0].Count != 1 {
		t.Fatalf("expected [{alice 1}], got %v", views)
	}
}
...
func TestMultipleSessions_ShowsCount(t *testing.T) {
	r := New()
	r.Register("alice", "user", false, "LOBBY")
	r.Register("alice", "user", false, "LOBBY") // second session, same user

	views := r.List("user")
	if len(views) != 1 {
		t.Fatalf("expected 1 entry for alice, got %d", len(views))
	}
	if views[0].Count != 2 {
...
func TestUnregister_DecrementsCount(t *testing.T) {
	...
	if len(views) != 1 || views[0].Count != 1 {
		t.Fatalf("expected [{alice 1}] after one unregister, got %v", views)
	}
}
...
func TestList_AlphabeticallySorted(t *testing.T) {
	...
	views := r.List("user")
	if views[0].Username != "alice" || views[1].Username != "alice" || views[2].Username != "zebra" {
...
func TestList_AdminInvisibleToRegularUser(t *testing.T) {
	...
	if len(views) != 1 || views[0].Username != "alice" {
